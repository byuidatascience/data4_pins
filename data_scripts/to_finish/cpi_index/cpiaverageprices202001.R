...
google_file <- drive_ls(sdrive) |>
  filter(stringr::str_detect(name, "cpiaverageprices202001"))
tempf <- tempfile()
drive_download(google_file, tempf) # Error: `file` does not identify at least one Drive file.
cpiaverageprices202001 <- read_excel(tempf)


# Publish the data to the server with Bro. Hathaway as the owner.
board <- board_connect()
pin_write(board, cpiaverageprices202001, type = "parquet", access_type = "all")

pin_name <- "cpiaverageprices202001"
