...
# %%
df = pl.read_csv('https://query.data.world/s/soqd6tbafx6r5f6puttwi5ahgiu5qw?dws=00000')
# https://stackoverflow.com/questions/72126913/polars-add-the-sum-of-some-columns-inside-select-with-column-call
# %%
df.group_by(["state_abb", "year", "name"])\
    .agg(pl.col("count").sum())\
    .pivot(values="count", index=["name", "year"],
        columns="state_abb", aggregate_function="first")\
    .with_columns(
        pl.all().exclude(["name", "year"]).sum().alias("Total"))\
    .fill_null(0)


#%%
# Read in data for gender and race probabilities
https://data.world/ssa/baby-names-for-us-states-territories/workspace/file?filename=baby-names-state.csv
